...
function News({ category }) {
  ...
  useEffect(() => {
    async function fetchNews() {
      ...
      setLoading(false);
      document.title = `Rick-News - ${category.charAt(0).toUpperCase() + category.slice(1)}`
    }
  ...
  const fetchMoreData = async () => {
    ...
    let url = `https://newsapi.org/v2/top-headlines?country=in&category=${category}&apiKey=${apiKey}&page=${page+1}&pageSize=15`;
    let data = await fetch(url);
    let parsedData = await data.json();
  ...
  return (
    <>
      <Navbar />
      <div className="flex justify-center align-middle m-12 mt-28"><h1 className="text-6xl">Top Headlines - {category.charAt(0).toUpperCase() + category.slice(1)}</h1></div>
      ...
      >
        { articles.map((element) => (
            <div key={element.url} id="main-content" className="w-96 m-3">
              <NewsItem
                title={element.title.length > 0 ? element.title.slice(0, 45) + "...": element.title}
                imageUrl={element.urlToImage}
                itemDesc={element.description? element.description.slice(0, 150) + "..." : ""}
                url={element.url}
